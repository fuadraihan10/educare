...
export default async function ParentTimetablePage() {
  ...
  const links = await prisma.studentGuardian.findMany({
    ...
    select: { student: { select: { firstName: true, lastName: true, classId: true } } },
  })
  ...
  const allTimetables = await Promise.all(
    links.map(async ({ student }) => {
      if (!student.classId) return { student, grouped: [] as { day: string; entries: Awaited<ReturnType<typeof listTimetableByClass>> }[] }
      const entries = await listTimetableByClass({ classId: student.classId })
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <PageHeader title="Timetable" subtitle="Class schedule for your children." />

      {allTimetables.map(({ student, grouped }) => (
        <div key={student.classId ?? `${student.firstName}-${student.lastName}`} className="space-y-4">
          <div className="flex items-center gap-3">
            <h2 className="text-lg font-semibold tracking-tight">{student.firstName} {student.lastName}</h2>
            <Badge variant="secondary" className="font-medium">{grouped.length} day{grouped.length !== 1 ? 's' : ''}</Badge>
          </div>

          {grouped.length === 0 ? (
            <EmptyState icon={CalendarDays} title="No timetable entries" description={`No timetable entries have been added for ${student.firstName}'s class yet.`} />
          ) : (
