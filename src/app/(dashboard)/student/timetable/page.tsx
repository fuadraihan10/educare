...
export default async function StudentTimetablePage() {
  ...
  const student = await prisma.student.findUnique({ where: { userId: user.id }, select: { classId: true } })
  if (!student?.classId) return <EmptyState icon={AlertCircle} title="No class assigned" description="You are not currently assigned to a class." />
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      <PageHeader title="My Timetable" subtitle="Your class schedule for the week." />

      {grouped.length === 0 ? (
        <EmptyState icon={CalendarDays} title="No timetable entries" description="No timetable entries have been added for your class yet." />
      ) : (
